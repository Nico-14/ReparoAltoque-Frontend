...
export default function Registrarse() {
    const onSubmit = (data) => {
        createAccount(data.type, data.email, data.password, data.name).then(
            (user) => {
                ...
                fetch(
                    process.env.NEXT_PUBLIC_API_URL + endpointRegitrarUsuario,
                    {
                        method: "post",
                        mode: "cors",
                        headers: {
                        ...
                        body: JSON.stringify({
                            ...
                            firebaseId: user.id,
                            lineOfWork: 1, // si es 1 es un cliente, 2 el rubro en el que trabaja no está en la lista, entre 3 y 44 es la lista de rubros actual
                        }),
                    }
                )
                    .then((res) => res.json())
